...
import {
  ...
  searchSemanticMemory, deleteSemanticMemory, getMemoryContext, getMarkdownMemory, saveMarkdownMemory, addMemoryFact, removeMemoryFact, getDailyNotes, getSessionSummaries, getMemoryAgentContext,
  ...
  getSupervisorStatus, getSupervisorLog, invokeSupervisorClaude, runHealthCheck, supervisorApprove, supervisorDiscard,
  analyzeWorkspace, autoCleanWorkspace, generateReadme,
  getSchedulerStatus, listSchedulerTasks, createSchedulerTask, deleteSchedulerTask, runSchedulerTaskNow, getSchedulerLog,
  listAutoSkills,
} from "../api";
...
export default function ControlCenter() {
  ...
  const [mdMemory,setMdMemory]=useState("");const [mdSections,setMdSections]=useState({});const [mdEditing,setMdEditing]=useState(false);const [mdEditContent,setMdEditContent]=useState("");
  const [dailyDates,setDailyDates]=useState([]);const [dailyContent,setDailyContent]=useState("");const [dailyDate,setDailyDate]=useState("");
  const [sessionSummaries,setSessionSummaries]=useState([]);const [agentContext,setAgentContext]=useState("");
  const [memTab,setMemTab]=useState("overview");const [newFactSection,setNewFactSection]=useState("Decisions");const [newFactText,setNewFactText]=useState("");
  ...
  async function refreshMemory(){try{const[ctx,md,daily,summaries,agCtx]=await Promise.all([getMemoryContext(),getMarkdownMemory(),getDailyNotes(),getSessionSummaries(),getMemoryAgentContext()]);setMemContext(ctx.context||null);setMdMemory(md.content||"");setMdSections(md.sections||{});setDailyDates(daily.dates||[]);setDailyContent(daily.content||"");if(daily.dates?.length)setDailyDate(daily.dates[0]);setSessionSummaries(summaries.sessions||[]);setAgentContext(agCtx.context||"")}catch{}}
  ...
  function renderMemory(){const ctx=memContext||{};const tabs=["overview","memory.md","daily","sessions","semantic"];return(<div style={{display:"flex",flexDirection:"column",gap:8}}>
    <h2>Memory</h2>
    {/* Tab bar */}
    ...
    {memTab==="memory.md"&&<div style={{display:"flex",flexDirection:"column",gap:8}}>
      ...
      {!mdEditing&&<>
        {Object.entries(mdSections).map(([sec,lines])=><div key={sec} className="card" style={{padding:10}}>
          ...
          {lines.filter(l=>l.trim()).map((l,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:4,fontSize:11,color:"var(--text-dim)",padding:"2px 0"}}>
            ...
            {l.trim().startsWith("-")&&<button className="btn-danger" style={{fontSize:9,height:18,padding:"0 4px"}} onClick={()=>act(async()=>{await removeMemoryFact(sec,l.replace(/^-\s*/,"").trim());await refreshMemory()},"Removed")}>✕</button>}
          </div>)}
        </div>)}
        {/* Add fact form */}
        <div className="card" style={{padding:10}}>
          ...
          <div style={{display:"flex",gap:4}}>
            ...
            <button className="btn-primary" disabled={!newFactText.trim()} onClick={()=>act(async()=>{await addMemoryFact(newFactSection,newFactText);setNewFactText("");await refreshMemory()},"Added")} style={{fontSize:10}}>Add</button>
          </div>
        </div>
      ...
      {mdEditing&&<div style={{display:"flex",flexDirection:"column",gap:4}}>
        ...
        <div style={{display:"flex",gap:4}}>
          <button className="btn-primary" onClick={()=>act(async()=>{await saveMarkdownMemory(mdEditContent);setMdEditing(false);await refreshMemory()},"Saved")} style={{fontSize:10}}>Save</button>
    ...
    <div className="card" style={{padding:16}}>
      ...
      <div style={{background:"var(--bg-root)",borderRadius:10,border:"1px solid var(--border)",minHeight:300,maxHeight:450,overflowY:"auto",padding:14,marginBottom:12,display:"flex",flexDirection:"column",gap:8}}>
        ...
        {svChat.map((m,i)=>{
          ...
          if(m.role==="action"&&m.data){const d=m.data;const pid=d.preview_id;
            if(d.type==="skill_preview")return(<div key={i} style={{alignSelf:"flex-start",maxWidth:"90%",padding:14,borderRadius:12,background:"var(--bg-elevated)",border:"1px solid var(--accent)",fontSize:12}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}><span style={{fontSize:18}}>{"\U0001f527"}</span><span style={{fontWeight:700,fontSize:14}}>New Skill: {d.spec?.name||d.spec?.tool_id}</span></div>
              ...
              {!m.resolved&&<div style={{display:"flex",gap:6}}>
                <button className="btn-primary" style={{height:28,fontSize:11}} onClick={async()=>{try{const r=await supervisorApprove(pid);setSvChat(c=>c.map((x,j)=>j===i?{...x,resolved:true,result:r}:x));toast(r.status==="success"?"Skill installed!":"Error: "+(r.error||""))}catch(e){toast(e.message,"error")}}}>Install</button>
                <button style={{height:28,fontSize:11}} onClick={()=>{supervisorDiscard(pid);setSvChat(c=>c.map((x,j)=>j===i?{...x,resolved:true,result:{status:"discarded"}}:x))}}>Discard</button>
              </div>}
            ...
            if(d.type==="agent_preview")return(<div key={i} style={{alignSelf:"flex-start",maxWidth:"90%",padding:14,borderRadius:12,background:"var(--bg-elevated)",border:"1px solid var(--info)",fontSize:12}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}><span style={{fontSize:18}}>{d.spec?.emoji||"\U0001f916"}</span><span style={{fontWeight:700,fontSize:14}}>New Agent: {d.spec?.name}</span></div>
              ...
              {!m.resolved&&<div style={{display:"flex",gap:6}}>
                <button className="btn-primary" style={{height:28,fontSize:11}} onClick={async()=>{try{const r=await supervisorApprove(pid);setSvChat(c=>c.map((x,j)=>j===i?{...x,resolved:true,result:r}:x));toast(r.status==="success"?"Agent created!":"Error")}catch(e){toast(e.message,"error")}}}>Create</button>
                <button style={{height:28,fontSize:11}} onClick={()=>{supervisorDiscard(pid);setSvChat(c=>c.map((x,j)=>j===i?{...x,resolved:true,result:{status:"discarded"}}:x))}}>Discard</button>
              </div>}
            ...
            if(d.type==="config_preview"||d.type==="command_preview"||d.type==="restart_preview")return(<div key={i} style={{alignSelf:"flex-start",maxWidth:"90%",padding:14,borderRadius:12,background:"var(--bg-elevated)",border:"1px solid var(--warning)",fontSize:12}}>
              <div style={{fontWeight:700,marginBottom:4}}>{d.type==="config_preview"?"\u2699\uFE0F Config Change":d.type==="command_preview"?"\u26A1 Command":"\U0001f504 Restart"}</div>
              <div style={{color:"var(--text-dim)",marginBottom:4}}>{d.command||d.setting||d.component}: {d.new_value||d.reason||""}</div>
              {!m.resolved&&<div style={{display:"flex",gap:6}}>
                <button className="btn-primary" style={{height:28,fontSize:11}} onClick={async()=>{try{const r=await supervisorApprove(pid);setSvChat(c=>c.map((x,j)=>j===i?{...x,resolved:true,result:r}:x));toast("Done")}catch(e){toast(e.message,"error")}}}>Approve</button>
                <button style={{height:28,fontSize:11}} onClick={()=>{supervisorDiscard(pid);setSvChat(c=>c.map((x,j)=>j===i?{...x,resolved:true,result:{status:"discarded"}}:x))}}>Cancel</button>
              </div>}
            ...
            if(d.type==="diagnosis")return(<div key={i} style={{alignSelf:"flex-start",maxWidth:"90%",padding:14,borderRadius:12,background:"var(--bg-elevated)",border:"1px solid "+(d.analysis?.severity==="high"?"var(--error)":"var(--warning)"),fontSize:12}}>
              <div style={{fontWeight:700,marginBottom:6}}>{"\U0001f50d"} Diagnosis</div>
  ...
  function renderScheduler(){return(<div style={{display:"flex",flexDirection:"column",gap:12}}>
    ...
    <div className="card" style={{padding:14}}>
      ...
      <div style={{display:"flex",gap:6,marginBottom:6}}>
        <select value={schForm.schedule} onChange={e=>setSchForm({...schForm,schedule:e.target.value})} style={{flex:1,height:28,fontSize:11}}>
      ...
      <button className="btn-primary" style={{width:"100%",height:30,fontSize:12}} onClick={async()=>{if(!schForm.description.trim())return;try{await createSchedulerTask(schForm);toast("Task created");setSchForm({description:"",schedule:"daily_09:00",action_message:"",agent_id:"",channel:""});listSchedulerTasks().then(r=>setSchTasks(r.tasks||[]))}catch(e){toast(e.message,"error")}}}>Create Task</button>
    </div>

    {schLog.length>0&&<div className="card" style={{padding:14}}>
      <h4 style={{margin:"0 0 8px"}}>Recent Executions</h4>
      {schLog.slice(-10).reverse().map((l,i)=><div key={i} style={{fontSize:11,padding:"3px 0",borderBottom:"1px solid var(--border)",display:"flex",gap:6}}>
  ...
  useEffect(()=>{if(activeSection==="skills")listAutoSkills().then(r=>setAutoSkills(r.skills||[])).catch(()=>{})},[activeSection]);

  const R={system:renderSystem,workspaces:renderWorkspaces,llm:renderLLM,metrics:renderMetrics,"self-improvement":renderSI,"auto-growth":renderAutoGrowth,mcp:renderMCP,a2a:renderA2A,memory:renderMemory,integrations:renderIntegrations,browser:renderBrowser,skills:renderSkills,supervisor:renderSupervisor,scheduler:renderScheduler,agents:renderAgents,"project-states":renderProjectStates};
